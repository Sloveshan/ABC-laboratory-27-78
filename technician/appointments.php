                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($row['technician_availability'] === 'yes' && $row['payment_status'] === 'Paid'): ?>
                                    <button class="btn btn-success approve-btn" disabled>Appointed</button>
                                <?php else: ?>
                                    <button class="btn <?php echo $row['technician_availability'] === 'yes' ? 'btn-secondary' : ($row['technician_availability'] === 'no' ? 'btn-warning' : 'btn-warning'); ?> approve-btn" data-appointment-id="<?php echo $row['appointment_id']; ?>" <?php echo $row['technician_availability'] === 'yes' ? 'disabled' : ''; ?>>
                                        <?php echo $row['technician_availability'] === 'yes' ? 'Approved' : 'Approve'; ?>
                                    </button>
                                <?php endif; ?>
                            </td>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['approve_id'])) {
        ...
        $stmt = $conn->prepare("UPDATE appointments SET technician_availability = 'yes', technician_id = ? WHERE appointment_id = ?");
        $stmt->bind_param("ii", $_SESSION['technician_id'], $appointmentId);
        $stmt->execute();
